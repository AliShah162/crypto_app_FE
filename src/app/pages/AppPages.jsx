...
export function Banner() {
  ...
  return (
    <div style={{ padding: "0 13px 12px" }}>
      <div style={{
        borderRadius: 19, background: sl2.g, height: 140,
        display: "flex", alignItems: "center", justifyContent: "center",
        gap: 17, padding: "0 20px", overflow: "hidden", position: "relative",
        boxShadow: "0 7px 26px rgba(0,0,0,0.5)", transition: "background 0.8s",
      }}>
        <div style={{
          position: "absolute", width: 160, height: 160, borderRadius: "50%",
          background: "rgba(255,255,255,0.025)", top: -52, right: -28,
        }} />
        {sl2.ico}
        <div>
          <div style={{ fontSize: 19, fontWeight: 900, color: "#fff" }}>{sl2.title}</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 3 }}>{sl2.sub}</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 8 }}>
        {[0, 1, 2].map((i) => (
          <div key={i} onClick={() => ss(i)} style={{
...
const qa = [
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", paddingBottom: 80 }}>
      <Banner />
      <div style={{ display: "flex", justifyContent: "space-around", padding: "4px 13px 18px" }}>
        {qa.map((a) => (
          <div key={a.l} onClick={() => nav(a.p)} style={{
            display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer",
          }}>
            <div style={{
              width: 52, height: 52, background: T.card2, borderRadius: 15,
              display: "flex", alignItems: "center", justifyContent: "center",
              border: `1px solid ${T.line}`,
            }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                stroke={a.c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d={a.d} />
              </svg>
            </div>
            <span style={{ fontSize: 11, color: T.dim, fontWeight: 500 }}>{a.l}</span>
          </div>
        ))}
      </div>
      <div style={{ fontSize: 14, fontWeight: 800, color: T.text, padding: "0 15px 9px" }}>Hot Trades</div>
      <div style={{ display: "flex", gap: 10, overflowX: "auto", scrollbarWidth: "none", padding: "0 13px 18px" }}>
        {COINS.map((c) => {
          const p = px[c.id] || 0;
          const h = PE.h[c.id] || [];
          ...
          return (
            <div key={c.id} onClick={() => nav("trade")} style={{
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: T.text }}>{c.id}</div>
              <div style={{ fontSize: 10, color: T.dim }}>{usd(p)}</div>
              <div style={{ fontSize: 10, fontWeight: 700, color: chg >= 0 ? T.green : T.red }}>
                {chg >= 0 ? "+" : ""}{f2(chg, 2)}%
              </div>
            </div>
          );
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 15px 9px" }}>
          ...
          return (
            <div key={c.id} style={{
              ...
              <svg width="48" height="20" viewBox="0 0 48 20">
                <polyline points={pts} fill="none" stroke={chg >= 0 ? T.green : T.red} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: T.text }}>{usd(p)}</div>
                <div style={{ fontSize: 10, fontWeight: 600, color: chg >= 0 ? T.green : T.red }}>
                  {chg >= 0 ? "+" : ""}{f2(chg, 2)}%
                </div>
              </div>
            </div>
      ...
      {txs.length > 0 && (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 15px 9px" }}>
            <span style={{ fontSize: 14, fontWeight: 800, color: T.text }}>Recent Transactions</span>
            <span onClick={() => nav("history")} style={{ fontSize: 11, color: T.acc, fontWeight: 700, cursor: "pointer" }}>All History</span>
          </div>
          <div style={{ padding: "0 13px 13px" }}>
            {txs.map((tx, i) => {
              ...
              return (
                <div key={i} style={{
                  background: T.card, borderRadius: 12, padding: "10px 13px",
                  display: "flex", alignItems: "center", gap: 11, marginBottom: 7,
                  border: `1px solid ${T.line}`,
                }}>
                  <div style={{
                    width: 38, height: 38, borderRadius: "50%",
                    background: tx.up ? "rgba(16,185,129,0.14)" : "rgba(239,68,68,0.14)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 15, color: m?.cl || T.acc, flexShrink: 0,
                  }}>{m?.sym || "$"}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: T.text }}>{tx.type}·{tx.coin}</div>
                    <div style={{ fontSize: 10, color: T.dim, marginTop: 1 }}>{tx.date}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: tx.up ? T.green : T.red }}>
                      {tx.up ? "+" : "-"}{usd(tx.usd)}
                    </div>
                  </div>
                </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingBottom: 20 }}>
      ...
      <div style={{ padding: "11px 13px" }}>
        {NEWS.map((n, i) => (
          <div key={i} style={{
            background: T.card, borderRadius: 13, padding: "14px 13px",
            marginBottom: 9, border: `1px solid ${T.line}`,
          }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: T.text, lineHeight: 1.5, marginBottom: 7 }}>{n.ttl}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: n.cl }}>{n.src}</span>
              <span style={{ fontSize: 10, color: T.dim }}>{n.ts}</span>
            </div>
            <div style={{ fontSize: 11, color: T.dim, lineHeight: 1.65 }}>{n.body}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", paddingBottom: 80 }}>
      <div style={{ padding: "17px 13px 9px" }}>
        <div style={{ fontSize: 19, fontWeight: 900, color: T.text, marginBottom: 11 }}>History</div>
        ...
        {list.length === 0 ? (
          <div style={{ textAlign: "center", color: T.dim, fontSize: 12, padding: "40px 0" }}>
            No {filt === "All" ? "transactions yet." : filt.toLowerCase() + " history yet."}
          </div>
        ) : (
          list.map((tx, i) => {
            const m = COINS.find((c) => c.id === tx.coin);
            return (
              <div key={i} style={{
                ...
                <div style={{
                  width: 38, height: 38, borderRadius: "50%",
                  background: tx.up ? "rgba(16,185,129,0.13)" : "rgba(239,68,68,0.13)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 15, color: m?.cl || T.acc, flexShrink: 0,
                }}>{m?.sym || "$"}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: T.text }}>{tx.type}·{tx.coin || "USD"}</div>
                  <div style={{ fontSize: 9, color: T.dim, marginTop: 1 }}>{tx.date}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: tx.up ? T.green : T.red }}>
                    {tx.up ? "+" : "-"}{usd(tx.usd)}
                  ...
                  {tx.amount && (
                    <div style={{ fontSize: 9, color: T.dim, marginTop: 1 }}>{f2(tx.amount, 4)} {tx.coin}</div>
                  )}
                </div>
              </div>
            );
    ...
    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", paddingBottom: 80 }}>
      <div style={{ padding: "18px 15px 0" }}>
        ...
        <div style={{
          ...
          borderRadius: 16, padding: "16px 15px", marginBottom: 13,
          boxShadow: "0 5px 18px rgba(0,0,0,0.4)",
        }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.45)", letterSpacing: 1, marginBottom: 4 }}>
            TOTAL PORTFOLIO VALUE
          </div>
          <div style={{ fontSize: 24, fontWeight: 900, color: "#fff", marginBottom: 11 }}>
            {usd(tot)}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 13 }}>
          {[
            {
              l: "Trades",
              v: String((liveUser?.transactions || []).filter((t) => t.type === "Buy" || t.type === "Sell").length),
            },
            {
              l: "Deposits",
              v: String((liveUser?.transactions || []).filter((t) => t.type === "Deposit").length),
            },
            {
              l: "Credit Score",
              v: creditScore >= 80 ? "🟢 " + creditScore : creditScore >= 50 ? "🟡 " + creditScore : "🔴 " + creditScore,
            },
            {
              l: "Country",
              v: country,
            ...
            <div key={s.l} style={{
              background: T.card, borderRadius: 12, padding: "11px 8px",
        ...
        {/* Menu */}
        <div style={{
          background: T.card, borderRadius: 15, padding: "4px 13px",
          border: `1px solid ${T.line}`, marginBottom: 11,
        }}>
          {menu.map((item, i) => (
            <div key={item.sp} onClick={() => onSub(item.sp)} style={{
              display: "flex", alignItems: "center", gap: 11, padding: "13px 0",
              borderBottom: i < menu.length - 1 ? `1px solid ${T.line}` : "none",
              cursor: "pointer",
            }}>
      ...
      <div style={{ padding: "18px 15px" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            margin: "0 auto 11px", fontSize: 26,
          }}>₿</div>
          <div style={{ fontSize: 19, fontWeight: 900, color: T.text, marginBottom: 3 }}>Coinbase AI-Quant</div>
          <div style={{ fontSize: 10, color: T.dim, letterSpacing: 2, fontWeight: 700 }}>VERSION: 2.1.0</div>
        </div>
        <div style={{
          background: T.card, borderRadius: 15, padding: "16px",
          border: `1px solid ${T.line}`, marginBottom: 13,
        }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: T.acc, marginBottom: 9 }}>
          ...
          {[
            ...
            <p key={i} style={{ fontSize: 11, color: T.dim, lineHeight: 1.75, marginBottom: 8 }}>{p}</p>
          ))}
        </div>
        <div style={{
          background: "linear-gradient(135deg,rgba(0,229,176,0.07),rgba(59,130,246,0.07))",
          borderRadius: 13, padding: "15px",
          border: "1px solid rgba(0,229,176,0.17)", textAlign: "center", marginBottom: 14,
        }}>
          <div style={{ fontSize: 10, color: T.dim, marginBottom: 4 }}>Our mission:</div>
          <div style={{ fontSize: 15, fontWeight: 900, color: T.text, lineHeight: 1.4 }}>
            Increase economic freedom for 1 billion+ people.
          </div>
        </div>
        <div style={{ textAlign: "center", fontSize: 10, color: T.dim, letterSpacing: 1 }}>
