...
import { PB, BHdr, Input, CoinIcon } from "../components/UI";
...
// ── Profile ────────────────────────────────────────────
export function ProfilePage({ nav, user, onLogout, onSub, re }) {
  const isBan = S.banned.has(user.username);
  const hVal = Object.entries(user?.holdings || {}).reduce(
    (s, [id, q]) => s + q * (PE.p[id] || 0),
    0,
  );
  const tot = (user?.balance || 0) + hVal;
  const menu = [
        ...
        >
          {[
            ...
            },
            {
              l: "Deposits",
              v: String(
                (user?.transactions || []).filter((t) => t.type === "Deposit")
                  .length,
              ),
            },
